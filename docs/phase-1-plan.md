...
**Model config:**
- Ingest → `default_model` (e.g. claude-sonnet-4-6)
- Query / Lint → `lightweight_model` (e.g. claude-haiku-4-5)
- API keys from env: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, etc.
...
okf_version: "0.1"
schema_pack: research          # personal | research | data-catalog
...
| `simple-git` | latest | Git operations including branches |
...
| **Default models** | Sonnet ingest, Haiku query/lint | Cost/quality balance |
