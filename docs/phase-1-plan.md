...
> Full personal/small-team breakdown: [Phase 2 Plan](phase-2-plan.md).

| Item | Phase |
|------|-------|
| `@memoss/catalog-bridge` (MaC sync) | 3+ (on demand) |
| `memoss enrich` | 2c (on demand) |
| `memoss discover` (local) | 2b |
| `memoss publish` / `memoss bridge` | 2b / 3 |
| `memoss sync` | 3+ |
| Hybrid vector search (`@memoss/search`) | 2b |
| Desktop / Web UI | deferred |
| BigQuery, Snowflake, dbt connectors | 3+ |
| Query `--format marp` | 2b |
| `memoss read` / Serial Read (`runSerialIngest`) | **2a (P0)** |
| [Serial Read Design](serial-read-design.md) | 2a |
