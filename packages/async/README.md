...
```typescript
import { retry, createRetry, withRetry, TimeoutError } from '@memoss/async';
...
- `@memoss/utils` (tagged with `scope:shared`)

Attempting to import from `@memoss/colors` or `@memoss/strings` will result in a linting error due to module boundary constraints.
